...
_lwp-request() {
    ...
    __lwp-request_handle_options_flags
...
function __lwp-request_handle_flags() {
    local i j
    declare -a copy
    local last="${MYWORDS[$INDEX]}"
    local max=`expr ${#MYWORDS[@]} - 1`
...
__lwp-request_handle_options_flags() {
    __lwp-request_handle_options
    __lwp-request_handle_flags
}
...
__comp_current_options() {
    ...
    if [[ -n $always || ${MYWORDS[$INDEX]} =~ ^- ]]; then
      ...
      __lwp-request_dynamic_comp 'options' "$options_spec"
...
complete -o default -F _lwp-request lwp-request
